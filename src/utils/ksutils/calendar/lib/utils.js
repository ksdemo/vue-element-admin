...
module.exports = {
   ...
   */
  getAllMonth: function(year) {
    year = year || this.getThisYear();
    var month = [31, null, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
    month[1] = this.isLeap(year) ? 28 : 29;
    return month;
  },
  ...
  getCurrentMonthTableData: function(d, isCompleteMonth) {
    ...
    var firstCell = firstDay.getDay()// === 0 ? 7 : firstDay.getDay(); // 周日则为7;
   ...
   * @return {String}
   */
  handleCalendarDisplayName: function(date, isVication) {
    var self = this;
  ...
  handleLastDay: function(maxDate) {
    ...
    if (str.indexOf('m') > -1) {
      ...
      var endYear = today.getFullYear() + Math.floor(interval / 12)
      var endMonth = today.getMonth() + interval % 12
      var endDay = this.getAllMonth(endYear)[endMonth];
      return new Date(endYear, endMonth, endDay);
    }
    // 按天
    else if (str.indexOf('d') > -1) {
      var itv = parseInt(str.replace('d', ''), 10);
      var thisDay = +new Date(today.getFullYear(), today.getMonth(), today.getDate()) + itv * 24 * 3600 * 1000;
